...
void WiFiManager::handleRoot() {
  ...
  String page = FPSTR(WP_HTTP_HEAD);
  page.replace("{v}", "Options");
  page += FPSTR(WP_HTTP_SCRIPT);
  page += FPSTR(WP_HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(WP_HTTP_HEAD_END);
  page += "<h1>";
  page += _apName;
  page += "</h1>";
  page += F("<h3>WiFiManager</h3>");
  page += FPSTR(WP_HTTP_PORTAL_OPTIONS);
  page += FPSTR(WP_HTTP_END);
...
void WiFiManager::handleWifi(boolean scan) {
    ...
    if (n == 0) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (_minimumQuality == -1 || _minimumQuality < quality) {
          String item = FPSTR(WP_HTTP_ITEM);
          String rssiQ;
          rssiQ += quality;
  ...
  for (int i = 0; i < _paramsCount; i++) {
    ...
    String pitem = FPSTR(WP_HTTP_FORM_PARAM);
  ...
  if (_sta_static_ip) {
    ...
    String item = FPSTR(WP_HTTP_FORM_PARAM);
    item.replace("{i}", "ip");
    item.replace("{n}", "ip");
  ...
  page += FPSTR(WP_HTTP_FORM_END);
  page += FPSTR(WP_HTTP_SCAN_LINK);
